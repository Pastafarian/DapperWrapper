...
namespace Logging.Serilog
{
    public class Logger<TImplementingType> : ILogger
    {
        ...
        {
            if (settings == null) throw new Exception("LoggerSettings parameter cannot be null");
            if (string.IsNullOrWhiteSpace(settings.LogFileLocation)) throw new Exception("LogFileLocation cannot be empty");
            if (string.IsNullOrWhiteSpace(settings.ApplicationName)) throw new Exception("ApplicationName cannot be empty");
        }
    }
}
